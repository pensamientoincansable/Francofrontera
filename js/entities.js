...
export class Entities {
  constructor(scene, fx, hooks) {
    this.scene = scene; this.fx = fx; this.hooks = hooks; // {fenceDamage, playerDamage, explode, groan, civDown, civSafe, allyShot, turretShot, turretKill, summon, bossDown}
  ...
  deployTurret() {
    ...
    const turret = {
      ...
      recoil: [0, 0],
      range: 48,
      baseYaw: Math.PI, // apunta al frente (hacia -Z)
    };
  ...
  buildZombie(type) {
    ...
    // Ojos hiperbrillantes y visibles a gran distancia
    ...
    // Marcador táctico de amenaza sobre la cabeza (visible al acercarse a la valla)
  ...
  update(dt, t, ctx) {
    ...
    for (const turret of this.turrets) {
      ...
      if (tgt) {
        ...
        // En espera: escaneo láser verde suave
    ...
    for (let i = this.list.length - 1; i >= 0; i--) {
      ...
      // Visibilidad reforzada: indicador táctico al acercarse a la valla
      if (z.threatMarker) {
      ...
      if (z.state === 'advance') {
        ...
        // En invasión, marchan hacia la base de la torre del francotirador
        ...
        if (prey) {
          ...
          // Asediando la torre de francotirador
